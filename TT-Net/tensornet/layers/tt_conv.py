...
def tt_conv(inp,         
    ...
    with tf.variable_scope(scope):# define computation graph for conv
        inp_shape = inp.get_shape().as_list()[1:]
        inp_h, inp_w, inp_ch = inp_shape[0:3]
        tmp = tf.reshape(inp, [-1, inp_h, inp_w, inp_ch])
        tmp = tf.transpose(tmp, [0, 3, 1, 2])#[batch size, channel, h, w], such as 64*3*h*w
        tmp = tf.reshape(tmp, [-1, inp_h, inp_w, 1]) #? (batch_size*3) * h * w * 1
        
        filters_shape = [window[0], window[1], 1, ranks[0]]# h * w * 1 * tt-filters ranks
        #get_var_wrap using to get desired tensor        
        if (window[0] * window[1] * 1 * ranks[0] == 1):#1 * 1 conv?
        ...
        # using modified filters to conv
        ...
        for i in range(d):
            ...
            cores.append(get_var_wrap('core_%d' % (i + 1),
                                      ...
                                      trainable=trainable,
                                      cpu_variable=cpu_variables))                                                    
        # Why two times convolution? have nn.conv then what's meaning for the multication for cores.
        for i in range(d):            
            tmp = tf.reshape(tmp, [ranks[i] * inp_ch_modes[i], -1]) # just reshape to accomplish mul? to fit the operation of tensor
            tmp = tf.matmul(cores[i], tmp) # out_ch_modes[i] * ranks[i + 1], ranks[i] * inp_ch_modes[i] * [ranks[i] * inp_ch_modes[i], -1]
            tmp = tf.reshape(tmp, [out_ch_modes[i], -1])
            tmp = tf.transpose(tmp, [1, 0]) # n * out_ch_modes[i]
    ...
    # output of convolution layer are batch_size * h * w * out_ch
    return out
