...
class tt_conv_direct(nn.Module):
    def __init__(self,
                    window, 
                    inp_ch_modes, 
                    out_ch_modes, \
        ...
        """
        super(tt_conv_direct, self).__init__()
        self.ranks = ranks 
        ...
        self.modes = [window[0], window[1], np.prod(inp_ch_modes), np.prod(out_ch_modes)]
        self.mat_cores = []
        for i in range(4):
            sz = self.modes[i] * ranks[i] * ranks[i + 1]
            if sz==1:
                cinit = torch.nn.init.ones_
            elif type(cores_initializer) == list:
                cinit = cores_initializer[i]
            else:
                cinit = cores_initializer
            w = torch.empty(ranks[i], ranks[i+1] * self.modes[i], device=device)
            cinit(w)
    ...
    def forward(self, x):
        ...
        full = self.mat_cores[0]
        for i in range(1,4):
            full = torch.reshape(full, [-1, self.ranks[i]])
            full = torch.matmul(full, self.mat_cores[i])
...
model = tt_conv_direct([3, 3],np.array([4,4,2,2],dtype=np.int32),np.array([4,4,4,4],dtype=np.int32),np.array([16,16,16,16,1],dtype=np.int32))
